...
class CodeGenerator:
    ...
    def __init__(self):
        ...
        # pointer for round robin float register fetch
        self.rr_registers_index_float = 0
        self.register_descriptor = self.default_reg_des.copy()  # register descriptor
        # float register descriptor
    ...
    def spill_register(self, register):
        ...
        if (self.address_descriptor[variable] is not None):
            try:
                ...
                if (self.symbol_table[variable]['datatype'] == Datatypes.INT):
                    self.text_segment += f"sw {register}, {-self.address_descriptor[variable]['offset']}(x8)\n"
                else:
                    self.text_segment += f"sb {register}, {-self.address_descriptor[variable]['offset']}(x8)\n"
            ...
            for line in block.splitlines():
                if (line.endswith(':')):
                    ...
                    if (line[-1].lower() == Datatypes.INT.value or line[-1].lower() == Datatypes.BOOL.value
                        ...
                        ld_command = 'lw' if exp_datatype.lower() == Datatypes.INT.value else 'lb'
                        ...
                        elif (line[5].lower() == Datatypes.INT.value):
                            self.update_symbol_table(
                                subject=line[0], datatype=Datatypes.INT)
                        elif (line[5].lower() == Datatypes.BOOL.value):
                            self.update_symbol_table(
                    ...
                    if (line[-1].lower() != Datatypes.FLOAT.value):
                        ...
                        if (constant):
                            ...
                            # CHAR
                                ...
                                self.text_segment += f"sb {lhs[0]}, {-offset}(x8)\n"
                                self.update_symbol_table(
                                    line[0], Datatypes.CHAR)
                            # STRING
                            elif (datatype == Datatypes.STR.value):
                                const = line[2]
                                var = line[0]
                                const = const[1:-1].replace(
                            ...
                            # STR
                                ...
                                self.data_segment_dict[line[0]] = [
                                    i for i in self.data_segment_dict[line[2]]]
                                self.data_segment_dict[line[0]
                                                       ][2] = self.data_segment_start
                    ...
                    # taking string input (INCOMPLETE)
                        ...
                        null_str = '"' + '\\0'*30+'"'
                        self.data_segment_dict[line[1]] = [
                            ".asciz", null_str, self.data_segment_start, 31]
                    ...
                    if (line[2].lower() == Datatypes.STR.value):
                        if (self.is_constant(line[1])[0]):
                            ...
                            total = hex(
                                self.data_segment_dict[var_name][2])[2:]
                            upper, mid, lower = self.get_ieee_rep(None, total)
