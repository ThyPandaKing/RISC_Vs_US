...
// import { returnPixelMap } from './Virtual_Display/pixel_map.js';
// import {bit_map} from './Virtual_Display/font_map.js';
...
window.addEventListener ('DOMContentLoaded', () => {
  ...
  // The following code runs periodically and refreshes the display with the latest data 
  // from the ../Device_Buffers/memory_map.txt file
  ...
  display_div.addEventListener ('click', () => {
    ...
    var interval = setInterval (async () => {
      ...
      hiddenDisplay.value = result_data;

      // pixel = returnPixelMap(result_data);
      const NUM_OF_ROW_PIXELS = 358;
      const NUM_OF_COL_PIXELS = 128;
      var pixel = '<div>';
      ...
      // Logic here to map each character to a bit map
      // const char_map = [0x18, 0x3C, 0x3C, 0x18, 0x18, 0x00, 0x18, 0x00];
      // const char_map = [0x1E, 0x33, 0x33, 0x1E, 0x33, 0x33, 0x1E, 0x00];

      var char_map = bit_mask[0]
      ...
      for (let index = 0; index < result_data.length; index++) {
        ...
        {
          let UTF_index = result_data.charCodeAt(index);
          char_map = bit_mask[UTF_index]
          for (let i = 0; i < char_map.length; i++) {
            for (let j = 0; j < 8; j++) {
              if ((char_map[i] & (1 << j)) != 0) {
                row[y_pos + i][x_pos + j] = 1;
              }
            }
          }
          x_pos = x_pos + 8;
        ...
        if (y_pos > (NUM_OF_COL_PIXELS - 8))
        {
          y_pos = 0;
        }
        if (x_pos > (NUM_OF_ROW_PIXELS - 8))
        {
          x_pos = 0;
          y_pos = y_pos + 8;
        }
        ...
        // }
        // x_pos += 8;
        // Add additional logic for newline character and change of y_pos
        
      }
      // for (let i = 0; i < result_data.length; i++) {
      //   //Looping through all the bits in the character
      //   for (let j = 0; j < 8; j++) {
      //     //Looping through all the pixels in the bit
      //     for (let k = 0; k < 8; k++) {
      ...
      display_div.innerHTML = pixel;
      // console.log('pixel map: ', pixel);
      // Re-render the HTML component with the updated contents
